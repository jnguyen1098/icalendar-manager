...
/* Converts JSON to DateTime struct */
DateTime *JSONtoDT(const char *str);

/* Converts a list of properties into JSON */
char *propListToJSON(List *propList);

/* Creates a new calendar using JSON strings and stores it somewhere */
int serverCreateCalendar(char *calJSON, char *evtJSON, char *calPath, int flag);

/* Takes an existing Calendar filename and returns the propList of an event */
char *filenameToList(char *fileName, int eventNum, int discrim);

/** Function to convert an alarm into a JSON string
 *@pre Alarm is not NULL
 *@return A string in JSON format
 *@param alarm - an Alarm struct
 **/
char *alarmToJSON(const Alarm *alarm);

/** Function to convert an Alarm list into a JSON string
